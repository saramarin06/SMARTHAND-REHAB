...
# CONTROL DE SESION
# =========================
# Boton fisico independiente para iniciar/finalizar una sesion.
# Primer clic: SESSION_START. Segundo clic: SESSION_END.
boton_sesion = Pin(4, Pin.IN, Pin.PULL_UP)
sesion_activa = False
EVENTO_NINGUNO = 0
EVENTO_START = 1
EVENTO_END = 2
evento_sesion = EVENTO_NINGUNO
ultimo_tiempo_sesion_irq = 0
serial_poll = select.poll()
serial_poll.register(sys.stdin, select.POLLIN)

def cambiar_sesion(pin):
    global sesion_activa, evento_sesion, ultimo_tiempo_sesion_irq
    tiempo_actual = ticks_ms()
    if ticks_diff(tiempo_actual, ultimo_tiempo_sesion_irq) > 300:
        sesion_activa = not sesion_activa
        evento_sesion = EVENTO_START if sesion_activa else EVENTO_END
        ultimo_tiempo_sesion_irq = tiempo_actual

boton_sesion.irq(trigger=Pin.IRQ_FALLING, handler=cambiar_sesion)

def iniciar_sesion():
    global sesion_activa, evento_sesion
    if not sesion_activa:
        sesion_activa = True
        evento_sesion = EVENTO_START

def finalizar_sesion():
    global sesion_activa, evento_sesion
    if sesion_activa:
        sesion_activa = False
        evento_sesion = EVENTO_END

def procesar_comandos_serial():
    # Permite iniciar/finalizar desde una interfaz en PC sin bloquear el ESP32.
    if serial_poll.poll(0):
        comando = sys.stdin.readline().strip()
        if comando == "START_SESSION":
            iniciar_sesion()
        elif comando == "END_SESSION":
            finalizar_sesion()
        elif comando == "TOGGLE_SESSION":
            if sesion_activa:
                finalizar_sesion()
            else:
                iniciar_sesion()
...
# =========================
while True:
    procesar_comandos_serial()

    if evento_sesion == EVENTO_START:
        rom1, rom2, rom3 = 0, 0, 0
        repeticiones1, repeticiones2, repeticiones3 = 0, 0, 0
        estado_flexion1, estado_flexion2, estado_flexion3 = 0, 0, 0
        print("SESSION_START")
        evento_sesion = EVENTO_NINGUNO
    elif evento_sesion == EVENTO_END:
        print("SESSION_END")
        evento_sesion = EVENTO_NINGUNO
